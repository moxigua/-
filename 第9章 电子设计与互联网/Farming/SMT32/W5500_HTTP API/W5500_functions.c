...
/*定义默认IP信息*/
uint8_t local_ip[4]={192,168,1,33};												/*定义W5500默认IP地址*/
uint8_t subnet[4]={255,255,255,0};												/*定义W5500默认子网掩码*/
uint8_t gateway[4]={192,168,1,1};													/*定义W5500默认网关*/


uint8_t server_tcp_ip[4]={192,168,1,106};												/*定义W5500 TCP服务器IP地址*/
...
//定义一个1s的定时器，供DHCP使用
static osTimerId DHCP_TimerID;
void DHCP_Timer_Function(void const *arg);
osTimerDef(DHCP_Timer,DHCP_Timer_Function);

/***********************************DNS的定义************************************/
//定义一个1s的定时器，供DNS使用
void DNS_Timer_Function(void const *arg);
static osTimerId osDNSTimerID;
osTimerDef(DNS_Timer,DNS_Timer_Function);

uint8_t dns_server[4]={114,114,114,114};									/*定义W5500默认DNS*/
uint8_t DNS_2nd[4]    = {114,114,114,115};      // Secondary DNS server IP
uint8_t hostIP[4]  = {0, };               // Translated IP address by DNS


/***********************************SNTP的定义************************************/
//定义一个1s的定时器，供DNS使用

uint8_t SNTP_Server[4]={210,72,145,44};	//国家授时中心服务器的IP地址
...
void DNS_Timer_Function(void const *arg)
 ...
 {  
	  // function return status
		osDNSTimerID = osTimerCreate(osTimer(DNS_Timer),osTimerPeriodic ,NULL);
		//每秒钟触发一次
		if(osDNSTimerID != NULL)
		{
			return 0;
		}
		else
		{
			 #ifdef Debug
				printf("osDNSTimer Started failed.");
				#endif
			return -1;
		}
 }
 ...
 int DNS_Timer_Start()
 {
	 if(osTimerStart(osDNSTimerID,1000) != osOK) //这不能传入0
		{
			return -1;
		}
		return 0;
 ...
 void DNS_Timer_Stop()
 {
	  osTimerStop(osDNSTimerID);
 }
 
 //把DNS放在IP中,//定义第几个socket来解析DNS
 int8_t DNS(uint8_t SOCK_DNS,  char *hostName, uint8_t *IP)
 {

	 int ret;
	   /* DNS client initialization */
   DNS_init(SOCK_DNS, buffer);
	 //DNS_Timer的定时器
   if((DNS_Timer_Start() || DNS_Timer_Start())) {
#ifdef Debug
					printf("DNS time start failure.\r\n");
#endif
		   return -2;
	 }
   /* DNS解析域名 */
   if ((ret = DNS_run(ConfigMsg.dns, (unsigned char *)hostName, IP)) > 0) //尝试首先DNS服务器解析
   {
		#ifdef Debug
					printf("> 1st DNS Reponsed\r\n");
		#endif
   }
   else if ((ret != -1) && ((ret = DNS_run(DNS_2nd, (unsigned char *)hostName, IP))>0)) //尝试备用服务器解析
   {
		#ifdef Debug
...
***************************************************************************************/
 ...
 int SNTP_GetTime(uint32_t timeOut)
 {
	  uint8_t b[51] = {0};
		int count = timeOut*10;
		while(count>0)
		{
			 DNS(6,"cn.pool.ntp.org",SNTP_Server);
			 SNTP_init(SNTP_Socket, SNTP_Server, 39,  b);
			 if(SNTP_run(&NowDatetime)){
				  timeSeconds = changedatetime_to_seconds();  //获得UTC时间
				 return 0;
			 }
			 osDelay(100);
			 count --;
